...
const tokenBalanceModelSpecificPrototype = {
  ...
  update: async function(params) {
    ...
    console.log('------deltaSB', (new BigNumber(deltaSB)));

    const balanceParams = {
      TableName: oThis.shardName,
      Key: oThis._keyObj({ethereum_address: params['ethereum_address'].toString()}),
      ...
      ExpressionAttributeValues: {
        ":deltaSB": 283497283497283497283497283497,//new BigNumber(deltaSB).toNumber(),
        ":deltaUD": 283497283497283497283497283497,//new BigNumber(deltaUD).toNumber(),
        ":delta_pessimistic_balance": 1
      },
      ReturnValues: "NONE"
    };
    ...
    if((new BigNumber(deltaUD)).gt(new BigNumber(0))){
      ...
      balanceParams['ExpressionAttributeValues'][':deltaUD'] = 1;
  ...
  set: async function(params) {
    ...
    if(settleAmount){
      ...
      ExpressionAttributeValues[":amount"] = settleAmount
    }
    if(unSettledDebitAmount){
      updateExpression.push(oThis.shortNameFor('unsettled_debits') +"=:unsettled_debit_amount");
      ExpressionAttributeValues[":unsettled_debit_amount"] = unSettledDebitAmount
    }
    if(pessimisticSettledBalance){
      ...
      ExpressionAttributeValues[":pessimistic_settled_balance"] = pessimisticSettledBalance
    }
  ...
  _keyObj: function (params) {
    ...
    keyObj[oThis.shortNameFor('ethereum_address')] = params['ethereum_address'].toString();
    keyObj[oThis.shortNameFor('erc20_contract_address')] = oThis.erc20ContractAddress.toString();
  ...
  _getDataFromDynamoInBatches: async function (ethereum_addresses) {
    ...
    for (let i=0; i < queryKeys.length; i++) {
      ...
      promiseArray.push(oThis.ddbServiceObj.batchGetItem(batchGetParams, 3));
