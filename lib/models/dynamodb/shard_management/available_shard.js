...
AvailableShard.prototype = {
  ...
  addShardTableEntry: function (ddbObject, shardTableName, entityType) {
    const oThis = this
      ...
      , insertItemParams = {
        ...
        Item: {
          [availableShardConst.SHARD_NAME]: shardTableName,
          [availableShardConst.ENTITY_TYPE]: entityType,
          [availableShardConst.ALLOCATION_TYPE]: availableShardConst.getShardTypes()[availableShardConst.disabled],
          [availableShardConst.CREATED_AT]: dateTime,
          [availableShardConst.UPDATED_AT]: dateTime
        }
      }
    ;

    return ddbObject.queryDocClient('put', insertItemParams);
  },
  ...
  configureShard: function (params) {
    const oThis = this
      ...
      , updateItemParam = {
        TableName: availableShardConst.getTableName(),
        Key: {
          [availableShardConst.SHARD_NAME]: shardName
        },
        ...
        ExpressionAttributeValues: {
          ":t": availableShardConst.ALLOCATION_TYPES[allocationType],
          ":u": (new Date().getTime())
        },
    ...
    return ddbObject.queryDocClient('update', updateItemParam);
  },
  ...
  getShardByName: async function (params) {
    ...
    ;
    const response = await ddbObject.queryDocClient('query', queryParams);
  ...
  hasShard: async function (params) {
    ...
    try {
      for (let ind = 0; ind < shardNames.length; ind++) {
        keys.push({
          [availableShardConst.SHARD_NAME]: String(shardNames[ind])
        });
      ...
      const response = await ddbObject.queryDocClient('batchGet', queryParams);
    ...
    try {
      for (let ind = 0; ind < ids.length; ind++) {
        ...
        let keyConditionExpression = "#et = :val1"
          ...
          , expressionAttributeValues = {
            ":val1": String(object.entity_type)
          }
        ;

        if (object.shard_type !== availableShardConst.all) {
          expressionAttributeValues[":val2"] = shardTypeCode;
        ...
        promiseArray.push(ddbObject.queryDocClient('query', queryParams));
      }
...
const Item = function (paramDdbItem) {
  ...
  defineProperty(oThis, "shardName",
    ddbItem[availableShardConst.SHARD_NAME]
  );

  defineProperty(oThis, "entityType",
    ddbItem[availableShardConst.ENTITY_TYPE]
  );

  defineProperty(oThis, "allocationType",
    availableShardConst.getInverseShardTypes()[ddbItem[availableShardConst.ALLOCATION_TYPE]]
  );

  defineProperty(oThis, "createdAt",
    ddbItem[availableShardConst.CREATED_AT]
  );

  defineProperty(oThis, "updatedAt",
    ddbItem[availableShardConst.UPDATED_AT]
  );
};
