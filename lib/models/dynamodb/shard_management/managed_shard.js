  ...
  getShard: async function (params) {
    ...
    try {
      for (let ind = 0; ind < identifiers.length; ind++) {
        ...
        ;
        keys.push({
          [managedShardConst.IDENTIFIER]: String(id),
          [managedShardConst.ENTITY_TYPE]: String(entityType)
  ...
  assignShard: function (params) {
    const oThis = this
      ...
      , insertItemParams = {
        ...
        Item: {
          [managedShardConst.IDENTIFIER]: identifier,
          [managedShardConst.ENTITY_TYPE]: entityType,
          [managedShardConst.SHARD_NAME]: shardName,
          [managedShardConst.CREATED_AT]: new Date().getTime(),
          [managedShardConst.UPDATED_AT]: new Date().getTime()
        }
      }
    ;
...
const Item = function (paramDdbItem) {
  ...
  defineProperty(oThis, "identifier",
    ddbItem[managedShardConst.IDENTIFIER]
  );

  defineProperty(oThis, "entityType",
    ddbItem[managedShardConst.ENTITY_TYPE]
  );

  defineProperty(oThis, "shardName",
    ddbItem[managedShardConst.SHARD_NAME]
  );

  defineProperty(oThis, "createdAt",
    ddbItem[managedShardConst.CREATED_AT]
  );

  defineProperty(oThis, "updatedAt",
    ddbItem[managedShardConst.UPDATED_AT]
  );
};
