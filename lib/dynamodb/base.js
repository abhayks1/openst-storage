...
Base.prototype = {
  ...
  /**
   * Query on DynamoDB using a raw DynamoDb connection object.
   *
   * @param methodName
   * @param params
  ...
  queryRaw: async function(methodName, ...params) {
    const oThis = this;
    let serviceType = dynamoFactory.raw;
  ...
  /**
   * Query on DynamoDB using a DocumentClient connection object.
   *
   * @param methodName
   * @param params
   * @returns {Promise<*|Promise<result>>}
  ...
  queryDocClient: async function(methodName, ...params) {
    const oThis = this;
    let serviceType = dynamoFactory.documentClient;
    const dbInstance = await dynamoFactory.getProvider(oThis.connectionStratgies, serviceType);
